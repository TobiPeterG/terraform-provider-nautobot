...
func resolveAvailableIPSource(ctx context.Context, c *nb.APIClient, prefixID, rangeID string) (string, string, string, error) {
	if (prefixID == "") == (rangeID == "") {
	...
	rng, httpResp, err := c.IpamAPI.IpamIpAddressRangesRetrieve(ctx, rangeID).Execute()
	if err != nil {
		return "", "", "", fmt.Errorf("failed to retrieve IP address range: %s", httpErr(err, httpResp))
	}
