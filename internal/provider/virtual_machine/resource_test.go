...
func testAccVirtualMachineConfigUpdatedWithoutTenant(name string) string {
	status := "Active"
...
func TestAccVirtualMachineResource_minimal(t *testing.T) {
	...
	name := fmt.Sprintf("tf-acc-vm-minimal-%d", testutil.AccSeedForTest(t))
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet(vmResourceName, "created"),
				),
			},
			{
...
func TestAccVirtualMachineResource_full(t *testing.T) {
	...
	name := fmt.Sprintf("tf-acc-vm-full-%d", testutil.AccSeedForTest(t))
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(vmResourceName, "tenant_id", "nautobot_tenant.test", "id"),
					resource.TestCheckResourceAttr(vmResourceName, "tags_ids.#", "0"),
				),
			},
			{
...
func TestAccVirtualMachineResource_update(t *testing.T) {
	...
	name := fmt.Sprintf("tf-acc-vm-update-%d", testutil.AccSeedForTest(t))
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccVirtualMachineConfigUpdatedWithoutTenant(name)},
			{
				Config: testutil.AccProviderConfig(),
			},
		},
...
func TestAccVirtualMachineResource_drift(t *testing.T) {
	t.Parallel()
	name := fmt.Sprintf("tfacc-vm-drift-%d", testutil.AccSeedForTest(t))
	config := testAccVirtualMachineConfigFull(name)
	var id string
	resource.Test(t, resource.TestCase{PreCheck: func() { testutil.AccPreCheck(t) }, ProtoV6ProviderFactories: testutil.AccProtoV6ProviderFactories, Steps: []resource.TestStep{
		{Config: config, Check: testutil.CaptureResourceID(vmResourceName, &id)},
		{PreConfig: func() {
			testutil.MutateResourceOutOfBand(t, id, "virtualization/virtual-machines", map[string]any{"comments": "outside Terraform"})
		}, Config: config, PlanOnly: true, ExpectNonEmptyPlan: true},
		{Config: config, Check: resource.TestCheckResourceAttr(vmResourceName, "comments", "created by terraform acceptance test")},
		{Config: testutil.AccProviderConfig()},
	}})
}
...
func TestAccVirtualMachineResource_import(t *testing.T) {
	...
	name := fmt.Sprintf("tf-acc-vm-import-%d", testutil.AccSeedForTest(t))
...
func TestAccVirtualMachineResource_delete(t *testing.T) {
	...
	name := fmt.Sprintf("tf-acc-vm-delete-%d", testutil.AccSeedForTest(t))
...
func TestAccVirtualMachineResource_deleteAlreadyGone(t *testing.T) {
	t.Parallel()
	name := fmt.Sprintf("tfacc-vm-del-gone-%d", testutil.AccSeedForTest(t))
	resource.Test(t, resource.TestCase{PreCheck: func() { testutil.AccPreCheck(t) }, ProtoV6ProviderFactories: testutil.AccProtoV6ProviderFactories, AdditionalCLIOptions: &resource.AdditionalCLIOptions{Plan: resource.PlanOptions{NoRefresh: true}}, Steps: []resource.TestStep{
		{Config: testAccVirtualMachineConfigMinimal(name), Check: testutil.DeleteResourceOutOfBand(vmResourceName, "virtualization/virtual-machines")},
		{Config: testutil.AccProviderConfig()},
	}})
}
...
func TestAccVirtualMachineResource_parallel(t *testing.T) {
	...
	baseName := fmt.Sprintf("tf-acc-vm-parallel-%d", testutil.AccSeedForTest(t))
